...
class Recommendations {
  ...
  async recommend (type) {
    try {
      ...
      const country = this.getCountryName(respJson.origin_country || respJson.country)
      return {
        score: score,
  ...
  async notify (entrypoint, msg, props) {
    try {
      ...
      switch (entrypoint) {
        case '/peli':
          promises.push(this.filmSuggestion(msg, props))
          break
        case '/serie':
          promises.push(this.showSuggestion(msg, props))
          break
        default:
          break
      }
      ...
      console.error('[Recommendations] Failed reading command.')
    }
  }
}
