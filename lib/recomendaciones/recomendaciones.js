...
module.exports = function (bot) {
  ...
  bot.on(['/dimeunapeli'], async (msg) => {
    try {
      const indice = getRandomInt.getRandomInt(0, 1000)
      ...
      if (typeof puntos === String) { puntuacion = 'sin puntuacion\n' } else if (puntos >= 7.5) { puntuacion = '🥇 Puntuación media: ' + puntos + '\n' } else if (puntos > 5) { puntuacion = '🥈Puntuación media: ' + puntos + '\n' } else if (puntos <= 5) { puntuacion = '🥉 Puntuación media: ' + puntos + '\n' }
  ...
  bot.on(['/dimeunaserie'], async (msg) => {
    try {
      const indice = getRandomInt.getRandomInt(0, 648)
      const pickOne = getRandomInt.getRandomInt(0, 19)
