...
module.exports = function (bot) {
    ...
    try {
      ...
      if (tmpP) {
        ...
        if (tmpAnswer) {
          ...
          if (!isThere) {
            if (acierto) { tmpAnswer.temas.push({tema: categoryName, aciertos: 1}) } else { tmpAnswer.temas.push({tema: categoryName, fallos: 1}) }
            await tmpAnswer.save()
          } else {
            if (acierto) { tmpAnswer.temas[index].aciertos++ } else { tmpAnswer.temas[index].fallos++ }
            await tmpAnswer.save()
          }
        } else {
          // crear una nueva respuesta
          let newAnswer
          if (acierto) { newAnswer = new Answer({userId: tmpP._id, temas: [{tema: categoryName, aciertos: 1}]}) } else { newAnswer = new Answer({userId: tmpP._id, temas: [{tema: categoryName, fallos: 1}]}) }
  ...
  const makeQuestion = async (id, category) => {
    try {
      ...
      if (!respJson || !respJson.incorrect_answers) { bot.sendMessage(msg.chat.id, 'Hubo error al procesar') }
      const arr = []
