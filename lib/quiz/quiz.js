...
module.exports = function (bot) {
  ...
  const response = async (msg, correct) => {
    try {
      ...
      if (tmpP) {
        ...
        if (tmpAnswer && tmpAnswer.temas && tmpAnswer.temas.length > 0) {
          // It has document
  ...
  const makeQuestion = async (id, category) => {
    try {
      ...
      if (!respJson || !respJson.incorrect_answers) { return bot.sendMessage(id, 'Hubo error al procesar') }
      const arr = []
  ...
  bot.on(['/clasificacion'], async (msg) => {
    try {
      ...
      if (user) {
        let tmpAnswer = await Answer.findOne({})
        let cadena = 'Puntuación de ' + user.username + '\n'
