...
module.exports = function(bot){
  ...
  bot.on(['/clasificacion'], async (msg) => {
    try {
      ...
      if (tmpP) {
        ...
        console.log(tmpAnswer.temas.length);
        // for(let tema in tmpAnswer.temas){
        //   cadena += 'Tema: '+tema.tema+'\n'+
  ...
  bot.on('/true', async msg => {
    ...
    if (tmpP) {
      let tmpAnswer = await Answer.findOne({userId:tmpP._id});
      if(tmpAnswer){
        let theTema = tmpAnswer.temas.map(function(o){return o.tema === categoryName});
        if(theTema)
          theTema.aciertos++;
        else{
        ...
        let newAnswer = new Answer({userId: tmpP._id, temas:[{tema:categoryName,aciertos:1}]});
  ...
  bot.on('/false', async msg => {
    ...
    if (tmpP) {
      ...
      if(tmpAnswer){
        ...
        let newAnswer = new Answer({userId: tmpP._id, temas:[{tema:categoryName,fallos:1}]});
        await newAnswer.save();
      }
